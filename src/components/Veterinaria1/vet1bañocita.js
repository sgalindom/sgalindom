...
const Vet1CitasScreen = () => {
  ...
  const obtenerFechasDisponibles = async () => {
    try {
      ...
      const citasSnapshot = await firestore().collection(`Administradores/${adminEmail}/citasbaños`).get();
  ...
  const obtenerUltimoId = async () => {
    try {
      ...
      const contadorIdRef = firestore().collection(`Administradores/${adminEmail}/citasbaños`).doc('contadorId');
  ...
  const obtenerHorasOcupadas = async (fecha) => {
    try {
      ...
      const horasSnapshot = await firestore()
        .collection(`Administradores/${adminEmail}/citasbaños`)
  ...
  const solicitarCita = async () => {
    try {
      ...
      // Obtener la colección de citasbaños
      const citasCollection = firestore().collection(`Administradores/${adminEmail}/citasbaños`);
...
export default Vet1CitasScreen;
