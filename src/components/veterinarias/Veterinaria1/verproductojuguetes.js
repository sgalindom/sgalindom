...
const VerProductoJuguetes = ({ route }) => {
  const navigation = useNavigation();
  const { producto } = route.params;
  const [cantidad, setCantidad] = useState(1);
  const [contadorProductos, setContadorProductos] = useState(0); // Contador de productos en el carrito
  ...
  useEffect(() => {
    const obtenerContadorProductos = async () => {
      try {
        ...
        const pedidoRef = usuarioRef.collection('pedidos').doc('juguetes').collection('items');

        // Obtener la cantidad de documentos en la colección de items
        const snapshot = await pedidoRef.get();
        setContadorProductos(snapshot.size);
      } catch (error) {
  ...
  const agregarAlCarrito = async () => {
    try {
      ...
      Alert.alert('Éxito', 'Producto añadido al carrito exitosamente.', [
        { text: 'OK', onPress: () => navigation.navigate('vet1juguetes') }
      ]);
    } catch (error) {
    ...
    <ImageBackground source={require('../../imagenes/fondomain.jpg')} style={styles.backgroundImage}>
      <View style={styles.container}>
        <View style={styles.productInfo}>
          <ImageBackground source={{ uri: producto.Foto }} style={styles.productImage}>
...
export default VerProductoJuguetes;
